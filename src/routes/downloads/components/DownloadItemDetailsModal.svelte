<script lang="ts">
  ...
  function isAudioStream(s: MediaTechnicalInfo['streams'][number]) {
    return s.codec_type === 'audio';
  }
  function isVideoStream(s: MediaTechnicalInfo['streams'][number]) {
  ...
  function formatHz(hz: number | undefined) {
        ...
        {#if item.filePath && !isAndroid()}
          ...
          {#if technicalLoading}
            <div class="media-skeleton" aria-hidden="true">
              <div
                class="s-row"
                use:skeleton={{ width: '70%', height: '14px', radius: '6px' }}
              ></div>
              <div
                class="s-row"
                use:skeleton={{ width: '55%', height: '14px', radius: '6px' }}
              ></div>
              <div
                class="s-row"
                use:skeleton={{ width: '85%', height: '14px', radius: '6px' }}
              ></div>
              <div
                class="s-row"
                use:skeleton={{ width: '60%', height: '14px', radius: '6px' }}
              ></div>
            </div>
          {:else if technicalError}
            <div class="hint error">{$t('downloads.details.media.failed')}</div>
          {:else if technicalInfo}
            {#if technicalInfo.format?.format_name}
              <div class="kv">
                <span class="k">{$t('downloads.details.media.container')}</span>
                <span class="v mono">
                  {technicalInfo.format.format_name}
                  {#if technicalInfo.format?.format_long_name}
                    <span class="muted"> ({technicalInfo.format.format_long_name})</span>
                  {/if}
                </span>
              </div>
            {/if}

            {#if technicalInfo.format?.bit_rate}
              <div class="kv">
                <span class="k">{$t('downloads.details.media.overallBitrate')}</span>
                <span class="v mono">{formatBitrate(technicalInfo.format.bit_rate)}</span>
              </div>
            {/if}
              ...
              <div class="kv">
                ...
                <span class="v">
                  <span class="mono">
                    ...
                    {#if s.codec_long_name}
                      <span class="muted"> • {s.codec_long_name}</span>
                    {/if}
                  </span>
                  {#if s.width && s.height}
                    <span class="muted"> • {s.width}×{s.height}</span>
                  {/if}
                  {#if parseFps(s.avg_frame_rate)}
                    <span class="muted">
                      • {$t('downloads.details.media.fps')}: {parseFps(s.avg_frame_rate)}</span
                    >
                  {:else if parseFps(s.r_frame_rate)}
                    <span class="muted">
                      • {$t('downloads.details.media.fps')}: {parseFps(s.r_frame_rate)}</span
                    >
                  {/if}
                  {#if s.bit_rate}
                    <span class="muted"> • {formatBitrate(s.bit_rate)}</span>
                  {/if}
                </span>
              </div>
            {/each}
            ...
            {#each technicalInfo.streams.filter(isAudioStream) as s (s.index)}
              <div class="kv">
                ...
                <span class="v">
                  ...
                  {#if s.sample_rate}
                    <span class="muted"> • {formatHz(s.sample_rate)}</span>
                  {/if}
                  {#if s.channels}
                    <span class="muted">
                  ...
                  {/if}
                  {#if s.channel_layout}
                    <span class="muted"> • {s.channel_layout}</span>
                  {/if}
                  {#if s.bit_rate}
                    <span class="muted"> • {formatBitrate(s.bit_rate)}</span>
                  {/if}
