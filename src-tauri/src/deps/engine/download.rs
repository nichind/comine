...
pub async fn download_file_with_checksum(
	...
	while let Some(chunk) = stream.next().await {
		...
		let chunk = chunk.map_err(|e| DepsError::Download(DownloadError::Request(e.to_string())))?;
	...
	info!("Download complete: {} bytes written to {:?}", actual_size, dest);

	if total_size > 0 && actual_size != total_size {
		error!("Size mismatch! Expected {} bytes, got {} bytes", total_size, actual_size);
		let _ = tokio::fs::remove_file(dest).await;
		return Err(DepsError::Download(DownloadError::size_mismatch(total_size, actual_size)));
	}
...
pub async fn fetch_json<T: serde::de::DeserializeOwned>(
	...
	Err(DepsError::UnsupportedPlatform("fetch_json not supported on Android"))
}
...
pub async fn fetch_text(
	...
	Err(DepsError::UnsupportedPlatform("fetch_text not supported on Android"))
}
...
pub async fn download_file_with_checksum(
	...
	Err(DepsError::UnsupportedPlatform("Downloading dependencies is not supported on Android"))
}
