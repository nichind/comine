		...
		if ffmpeg_path.exists() {
			match run_capture_async(&ffmpeg_path, &["-version"]).await {
				Ok(output) if output.status_code == Some(0) => {
					...
					let disk_size = {
						let ffmpeg_size = tokio::fs::metadata(&ffmpeg_path).await.ok().map(|m| m.len()).unwrap_or(0);
						let ffprobe_size = tokio::fs::metadata(&ffprobe_path).await.ok().map(|m| m.len()).unwrap_or(0);
...
pub async fn install_ffmpeg(
	...
	#[cfg(any(target_os = "windows", target_os = "macos", target_os = "linux"))]
	{
		...
		let expected_sha256 = try_fetch_sha256(&checksum_urls, &config, Some("ffmpeg"))
			.await;
